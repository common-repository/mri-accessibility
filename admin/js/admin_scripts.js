jQuery(document).ready(function($) {
    ...
    //Toggle admin section ORG Admin
    jQuery(".form_element_header").click(function(e){
        e.preventDefault();
        var this_el     = jQuery(this);
        var toggle_span = jQuery(this).find("span.toggle-org-section span.dashicons");
        if(toggle_span.hasClass('dashicons-arrow-down-alt2')){
            ...
            this_el.next(".org_form_elements_wrapper").slideUp(200);
        } else {
            toggle_span.removeClass("dashicons-arrow-up-alt2");
            toggle_span.addClass('dashicons-arrow-down-alt2');
            this_el.next(".org_form_elements_wrapper").slideDown(200);
        }
    });
    ...
    jQuery("#org_enable_custom_contrast").change(function(){
    ...
    jQuery(".attachment_post_title").change(function(){
        ...
        var data = {
            action: 'miah_update_attachment_title',
            pid:    pid,
            ptitle: ptitle,
        };
    ...
    jQuery(".attachment_post_alt").change(function(){
       ...
       var data = {
           action: 'miah_update_attachment_alt',
           pid:    pid,
           palt: palt,
       };
       jQuery.post(ajaxurl, data, function(response) {
    ...
    //ORG SCANNER
    jQuery("#org_scanner").click(function(e){
        e.preventDefault();
        var postID = jQuery("#org_scanner_selector").val();
        if(postID){
            ...
            jQuery.ajax({
                ...
                dataType : "json",
                url : ajaxurl,
                data : { action: "miah_scan_homepage", postID : postID },
                success: function(response) {
                    if(response.response_code == 200){
                        jQuery("#org_scanner_results").php('');
                        if(response.images){
                           jQuery("#org_scanner_results").append(response.images);
                        }
                        if(response.links){
                           jQuery("#org_scanner_results").append(response.links);
                        }
    ...
    jQuery("body").on("click",".org_scanner_table_trigger",function(event){
        event.preventDefault();
        jQuery(this).toggleClass("active");
        jQuery(this).next(".org_scanner_table").slideToggle(300);
    });
    //Save org widgets order
    jQuery( "#sortable-org-widget" ).sortable({
        placeholder: "ui-state-highlight",
        update: function( event, ui ) {
            jQuery("#fountainG").fadeIn(50);
            var neworder = [];
            jQuery('#sortable-org-widget li').each(function() {
            ...
            if(neworder){
                jQuery.ajax({
                    ...
                    url         : ajaxurl,
                    data        : {
                        action  : "org_update_widgets_order",
                        alldata : neworder
                    },
                    success: function(response) {
                        if(response == 'ok'){
    ...
    jQuery("body").on("click","button.org-button.delete-contrast-params",function(e){
        e.preventDefault();
        ...
        jQuery.ajax({
            ...
            url         : ajaxurl,
            data        : {
                action  : "miah_remove_contrast_item"
            },
            success: function(response) {
...
function add_new_contrast_item(){
    jQuery("button.org-button.org-add-item").click(function(e){
        e.preventDefault();
        ...
        if(total_contrast_elements >= 5) {
            ...
            jQuery(".org-contrast-loader").fadeTo(100,1);
            jQuery.ajax({
                type        : "post",
                dataType    : "json",
                url         : ajaxurl,
                data        : {
                    action  : "miah_add_new_contrast_item"
                },
                success: function(response) {
                    if(response.status == 'ok' && response.php){
                        jQuery("ul.contrast-params-list").append(response.php);
                        jscolor.installByClassName("jscolor");
                        jQuery(".org-contrast-loader").fadeTo(100,0);
                    }
                },
                error: function(response){
                }
        ...
        if( jQuery('ul.contrast-params-list li').length ){
            ...
            if( contrast_variations ){
                ...
                jQuery.ajax({
                    ...
                    url         : ajaxurl,
                    data        : {
                        action  : "miah_save_contrast_variations",
            ...
            jQuery.ajax({
                ...
                url         : ajaxurl,
                data        : {
                    action  : "miah_save_empty_contrast_variations"
                },
                success: function(response) {
                    if(response.status == 'ok'){
