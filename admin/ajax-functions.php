<?php
add_action( 'wp_ajax_update_attachment_title', 'miah_update_attachment_title' );
function miah_update_attachment_title() {
...
add_action( 'wp_ajax_update_attachment_alt', 'miah_update_attachment_alt' );
function miah_update_attachment_alt() {
...
//ORG SCANNER
add_action('wp_ajax_org_scan_homepage','miah_scan_homepage');
function miah_scan_homepage(){
	...
	if( $body && $postID && $response_code == 200 ){
		...
		if($scanner_array['images']){
			...
			<div class="form_row">
				<h3 class="org_scanner_table_trigger">
					<span></span><?php _e("Images Report Table","wp-accessibility-helper"); ?>
				</h3>
				<table class="widefat fixed org_scanner_table" cellspacing="5">
					<thead>
						<tr>
							<th class="manage-column column-cb check-column org_th" scope="col" style="width:130px !important;">
								<?php _e("Thumbnail","wp-accessibility-helper"); ?>
							</th>
							<th class="manage-column image_src">
								<?php _e("Image Source","wp-accessibility-helper"); ?>
							</th>
							<th class="manage-column column-cb check-column org_th" scope="col">
					...
					<tbody>
						<?php foreach($scanner_array['images'] as $key=>$image_html):
							...
							$attachment_id = miah_get_attachment_id_by_src($src);

						?>
						<tr>
						<td class="org_scanner_thumbnail"><?php echo $image_html; ?></td>
						<td>
						...
						<td>
							<a target="_blank" href="<?php echo add_query_arg('orgi',base64_encode($src), get_permalink($postID));?>">
								<?php _e("Preview","wp-accessibility-helper"); ?>
							</a>
						</td>
						</tr>
		...
		<div class="form_row">
			...
			<table class="widefat fixed org_scanner_table" cellspacing="5">
				<thead>
					<tr>
						<th class="manage-column column-cb check-column org_th" scope="col" style="width:26px;">
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td class="org_scanner_thumbnail">
							...
							<td>
								<a href="<?php echo add_query_arg('orgl',base64_encode($href), get_permalink($postID));?>" target="_blank">
...
function miah_get_attachment_id_by_src($image_url){
	global $wpdb;
...
**************************************************/
add_action('wp_ajax_org_update_image_alt','miah_update_image_alt');
add_action('wp_ajax_nopriv_org_update_image_alt','miah_update_image_alt');
function miah_update_image_alt(){

    $response = array();
    $attachment_source = isset($_POST['target_src']) ? sanitize_text_field($_POST['target_src']):       '';
	$org_alt_input     = isset($_POST['org_alt_input']) ? sanitize_text_field($_POST['org_alt_input']): '';

    if( $attachment_source ) {

        $attachment_id = miah_get_attachment_id( $attachment_source );

        if($attachment_id && $org_alt_input){
			update_post_meta($attachment_id, '_wp_attachment_image_alt', $org_alt_input);
...
/*****************************************************
*	Save org widgets order
*****************************************************/
add_action( 'wp_ajax_org_update_widgets_order', 'miah_update_widgets_order' );
function miah_update_widgets_order(){

	$response       = '';
	$data           = isset($_POST['alldata']) ? sanitize_text_field($_POST['alldata']): '';
	...
	update_option('org_sidebar_widgets_order', $s_data);
	$response = 'ok';
...
*****************************************************/
add_action( 'wp_ajax_add_new_contrast_item', 'miah_add_new_contrast_item' );
function miah_add_new_contrast_item(){
	$response = array();
	ob_start();
?>
	<li>
...
*****************************************************/
add_action( 'wp_ajax_remove_contrast_item', 'miah_remove_contrast_item' );
function miah_remove_contrast_item(){
	$response = array();
	$response['status'] = 'ok';
	echo json_encode($response);
	die();
...
****************************************************/
add_action( 'wp_ajax_save_contrast_variations', 'miah_save_contrast_variations' );
function miah_save_contrast_variations(){
	$response = array();
	$alldata  = isset($_POST['alldata']) ? sanitize_text_field($_POST['alldata']): '';
	if( count($alldata) >= 5 ){
		$response['status'] = 'error';
		...
		if($alldata){
			...
			update_option('org_contrast_variations',$data);
			$response['status'] = 'ok';
			echo json_encode($response);
		}
	}
...
****************************************************/
add_action( 'wp_ajax_save_empty_contrast_variations', 'miah_save_empty_contrast_variations' );
function miah_save_empty_contrast_variations(){
	$response = array();
	$alldata  = '';
	update_option('org_contrast_variations',$alldata);
	$response['status'] = 'ok';
...
****************************************************/
function miah_get_contrast_variations(){
